...
//	___________________________
	enableScreenpadControl() {
		...
		// Declaration des objects quickSetting
		...
		// Declaration des objects et autre stream Gio
		...
		// --------------------------------------------------------------------------------- //
		// 		Création du GIO File pour 	/sys/class/backlight/asus_screenpad/brightness.	 //
		...
		// ATTENTION
		// Je déconnecte le MainSlider lorsque screenpadSlider est activé
		...
		this._screenpadSliderDragEndId 			= this._screenpadSlider.slider.connect('drag-end',
				() => 	{ 
							...
							this._mainSliderChangedId = this.mainSlider.slider.connect('notify::value',
								() => this._onMainSliderChanged() );
						});

		this._screenpadSliderChangedId 			= this._screenpadSlider.slider.connect('notify::value',
				() => this._onScreenpadSliderChanged() );
		...
		this._screenpadToggleSwitchId 		= this._screenpadToggleMenu.connect( 'clicked', 
				() =>	this._onScreenPadToggleSwitchClicked() );

		this._screenpadToggleMenuId		=this._settings.connect('changed::screenpad-mode', 
				() =>	this._onScreenpadToggleMenuChange() );
...
//--------------------------------------------------------------------------------------------- //
	disableScreenpadControl() {
		...
		// Deconnexion du _screenpadSlider
	...
	_onScreenpadSliderChanged() {
		...
		// si le screenpad est Off, alors 0
		//			c'est ma facon de déconnecter le slider
		//			Je n'est pas trouvé la fonction this._screenpadSlider.slider.INACTIVE()
		if (this._settings.get_string('screenpad-mode') == "Off") {
			SetScreenPadBrightness = 0;
			this._screenpadSlider.slider.value = 0;
		}

		// si le screenpad est Full, alors 1
		//			c'est ma facon de déconnecter le slider
		//			Je n'est pas trouvé la fonction this._screenpadSlider.slider.INACTIVE()
	...
	_onMainSliderChanged() {
		...
		// Jamais inferieur à 1
		// C'est Maintenant possible avec le nouveaux noyeau
//		if (MainBrightness < 1) MainBrightness = 1;
...
//				this._settings.set_strv	('screenpad-last-mode', [	this._settings.get_strv('screenpad-last-mode')[0],
//																	this._settings.get_int('screenpad-brightness').toString() ]
//										);
...
//				this._settings.set_strv('screenpad-last-mode', [this._settings.get_string('screenpad-mode')]);
	...
	// 		Mais attention, l'evenement _screenpadSliderDragBeginId n'a pas lieux!				 //
	// 		Le RW Stream n'est pas ouvert															 //
	// 		Pareil pour _screenpadSliderDragEndId et la fermeture du RW Stream						 //
	// --------------------------------------------------------------------------------------------- //
